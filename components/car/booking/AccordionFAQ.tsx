...
function Accordion(props: accordionProps & PropsWithChildren) {
	const [visible, setVisible] = useState(false);

	return (
		<div className="w-full min-w-[350px] rounded-3xl bg-darkgray flex flex-col border border-halfblack cursor-pointer select-none mb-5">
			<div
				className="w-full h-[80px] rounded-3xl bg-darkgray text-white font-semibold  text-3xl flex p-5 items-center"
				onClick={() => setVisible(!visible)}>
				<div className="flex flex-row justify-between w-full">
					<span className=" font-medium text-xl">{props.header}</span>
			...
			{visible && (
				<div className="block mx-8 mb-8 text-halfblack">
					{props.faq?.map((el, id) => {
						return (
							<div key={id} className="flex flex-row justify-start mb-4">
								<img src={el.title} className=" mr-3" />
								<span className=" font-medium text-base">{el.data}</span>
							</div>
						);
					})}
				</div>
...
export default function AccordionFAQ(props: accordionProps & PropsWithChildren) {
	const [visible, setVisible] = useState(false);
	return (
		<section className="max-[1220px]:flex-grow min-[1220px]:w-[380px]">
			<Accordion
				header="Условия"
				faq={[
					{
