...
export default function SlideLine({
  ...
  const slides = displayedSrcs.map((slide, id) => {
    return (
      <div
        className={`relative cursor-pointer aspect-[3/2] w-full ${
          id === _index && "bg-background opacity-20"
        }`}
        key={`${id + slide?.url}`}
      >
        <Image
          className="rounded-[16px] border border-black"
          fill
          src={slide?.url}
          alt=""
          style={{
  ...
  return (
    <div className="flex flex-row justify-between w-full h-[15vw] max-h-[120px] gap-5 mt-5">
      {slides}
    </div>
  );
}
