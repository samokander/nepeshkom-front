...
export default function Options(props: {
  ...
  const [startRentHour, setStartRentHour] =
    useState<string>();
  const [endRentHour, setEndRentHour] = useState<string>();
  ...
  const handleRentAuto = async () => {
    if (!getLocalStorageClientData().phoneNumber)
      router.push("/auth");
    if (fromValue && toValue) {
      const startDate =
        convertDateFormat(fromValue) +
        " " +
        (startRentHour || "00:00") +
        ":00";
      const endDate =
        convertDateFormat(toValue) +
        " " +
        (endRentHour || "00:00") +
        ":00";
      const res = await addRentRequest(
        props.autoId,
        startDate,
        endDate
      );

      if (typeof res?.LongParamValue === "number")
        setErr(false);
      else setErr(true);
    } else {
      setErr(true);
    ...
    <section className="flex flex-col rounded-[16px] p-[20px] bg-darkgray w-[380px] border border-[#5B5B5B] h-auto">
      ...
      <span className="mb-[20px] font-semibold text-[16px] text-white opacity-50">
        ...
        {!(toValue && fromValue)
          ? 0
          : Math.floor(
              (new Date(toValue).valueOf() -
                new Date(fromValue).valueOf()) /
                (1000 * 60 * 60 * 24)
            )}
      </span>
