...
export default function Filter(props: {
	...
	async function handleFilterClick() {
		...
		const autos = (await axios.get(process.env.NEXT_PUBLIC_ALL_AUTOS_ROUTE as string)).data as AutoCard[];
		props.setAutos(autos.filter((el) => filterFunction(el)));
				...
				{showFilter && (
					<div className="flex flex-row justify-evenly absolute w-full h-24 top-full bg-darkgray  border-x border-x-halfblack border-b border-b-halfblack items-center justify-items-stretch gap-5 px-5 rounded-b-2xl">
						...
						<select
							...
							value={classCode}
							onChange={(e) => setClasssCode(e.target.value)}>
							<>
								<option disabled hidden value="">
									Класс
								</option>
								{filters?.autoClassesCodes.map((el, index) => {
									return (
										<option value={el} key={index}>
											{el}
										</option>
